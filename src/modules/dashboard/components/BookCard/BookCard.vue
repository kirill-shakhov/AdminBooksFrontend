<template>
  <router-link :to="{ name: 'Book', params: { bookId: _id } }"
               class="relative flex w-full max-w-[18rem] flex-col rounded-xl bg-white bg-clip-border text-gray-700 shadow-lg h-full ">
    <div
      ...
      <button
          class="!absolute  top-4 right-4 h-8 max-h-[32px] w-8 max-w-[32px] select-none rounded-full text-center align-middle font-sans text-xs font-medium uppercase  transition-all hover:bg-red-500/10 active:bg-red-500/30 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
          type="button">
        <!--        text-red-500-->
    ...
    <div class="p-6">
      ...
      <p class="block  text-transform: capitalize font-sans text-base antialiased font-light leading-relaxed text-gray-700">
        {{ author.name }}
      </p>
